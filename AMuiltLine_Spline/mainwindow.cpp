...
void MainWindow::QLineAddPoint(QString lineName,double dPoint){
    ...
    chartLineMutex.lock();
    // 添加节点值
    QMap<QSplineSeries*,QList<double>*> line = chartLines[lineName];
    // 添加值
...
void MainWindow::ChartAddQLine(QString lineName){
    ...
    if(!chartLines.contains(lineName)){
        chartLineMutex.lock();
        // 添加新的曲线
        QMap<QSplineSeries*,QList<double>*> newLine;
...
void MainWindow::ChartRemoveQLine(QString lineName){
    ...
    // 添加节点值
    QMap<QString,QMap<QSplineSeries*,QList<double>*> >::Iterator it = chartLines.find(lineName);

    QMap<QSplineSeries*,QList<double>*> itval = it.value();
    // 添加值
    QSplineSeries *line = itval.begin().key();
...
void MainWindow::ChartVisibleLine(QString lineName,bool visible){
    ...
    QMap<QSplineSeries*,QList<double>*> lIt = chartLines[lineName];
    QSplineSeries *line = lIt.begin().key();
...
void MainWindow::ChartUpdateQLine(){
    ...
    chartLineMutex.lock();
    QMap<QString,QMap<QSplineSeries*,QList<double>* > >::Iterator it;    
    for(it = chartLines.begin();it != chartLines.end();it++){
        QString lineName = it.key();
        QSplineSeries *line = it.value().begin().key();
...
{
    static double a = 0.0;
    QLineAddPoint("sin", 200.0 * sin(a+=0.2));
    QLineAddPoint("cos", 200.0 * cos(a+=0.2));
